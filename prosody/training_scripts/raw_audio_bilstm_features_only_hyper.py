...
def objective(trial):
    ...
    NUM_ATTENTION_LAYERS = trial.suggest_int(
        'NUM_ATTENTION_LAYERS', 2, 8, step=2)
    ...
    sample_words, sample_features, sample_labels, \
        sample_lengths = next(iter(train_loader))
    ...
    encoder = Encoder(
        feature_dim, HIDDEN_DIM, NUM_LAYERS, DROPOUT).to(device)
    decoder = Decoder(
        HIDDEN_DIM, num_classes, NUM_LAYERS, DROPOUT,
        NUM_ATTENTION_LAYERS).to(device)
    model = Seq2Seq(encoder, decoder).to(device)

    optimizer = optim.Adam(
        model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, 'min', factor=0.1, patience=5, verbose=False)
    ...
    for epoch in range(N_EPOCHS):
        ...
        valid_loss, valid_acc, valid_precision, valid_recall, \
            valid_f1 = evaluate(model, val_loader, criterion)
    ...
    return best_valid_f1  # Return validation F1 score

if __name__ == "__main__":
    seed = 42
    set_seed(seed)

    json_path = '../prosody/data/multi_label_extracted_audio_features.json'
    ...
    study.optimize(objective, n_trials=50)  # Increase n_trials
    ...
    for epoch in range(N_EPOCHS):
        train_loss = train(model, train_loader, optimizer, criterion)
        valid_loss, valid_acc, valid_precision, valid_recall, \
            valid_f1 = evaluate(model, test_loader, criterion)
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(
                model.state_dict(),
                'models/best-model-raw-audio-features-version.pt')

        print(f'Epoch: {epoch+1:02}')
        print(f'\tTrain Loss: {train_loss:.3f}')
        print(f'\t Val. Loss: {valid_loss:.3f} | Val. Acc: {valid_acc*100:.2f}%')
        print(f'\t Precision: {valid_precision:.2f} | Recall: {valid_recall:.2f}')
    ...
    model.load_state_dict(
        torch.load('models/best-model-raw-audio-features-version.pt'))
