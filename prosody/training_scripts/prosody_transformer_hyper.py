...
if __name__ == "__main__":
    ...
    train_data, val_data, test_data = split_data(data, train_ratio=0.9, val_ratio=0.05, test_ratio=0.05)
    check_overlap(train_data, val_data, test_data)

    # Fit scaler on training data for feature normalization
    temp_train_dataset = ProsodyDataset(train_data)
    scaler = fit_feature_scaler(temp_train_dataset)
    ...
    # Determine number of classes based on unique labels across datasets
    all_unique_labels = get_all_unique_labels([train_dataset, val_dataset, test_dataset])
    NUM_CLASSES = len(all_unique_labels)
    ...
    # Initialize the Transformer model with best hyperparameters from Optuna
    model = TransformerClassifier(
        feature_dim=feature_dim,
    ...
    # Validate labels to ensure they are within the expected range
    validate_labels([train_dataset, val_dataset], NUM_CLASSES)
    ...
    # Define optimizer with learning rate and weight decay from Optuna
    optimizer = optim.Adam(model.parameters(), lr=trial.params['lr'], weight_decay=trial.params['weight_decay'])
    # Define learning rate scheduler to reduce LR on plateau
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=3, min_lr=1e-6)

    # Define loss function with class weights and ignoring the padding index
    criterion = nn.CrossEntropyLoss(ignore_index=PADDING_VALUE, weight=class_weights)

    # Training parameters
    N_EPOCHS = 50

    # Initialize early stopping with patience and minimum delta
    early_stopping = EarlyStopping(patience=10, min_delta=0.0001)
    best_valid_f1 = 0.0

    # Ensure the models directory exists for saving the best model
    os.makedirs('models', exist_ok=True)
    best_model_filename = 'models/best_transformer_model.pt'
    ...
    for epoch in range(N_EPOCHS):
        ...
        # Early stopping check (negative F1 because EarlyStopping expects a value to minimize)
        ...
        print(f'Epoch: {epoch+1:02} | Learning Rate: {optimizer.param_groups[0]["lr"]:.6f} | '
    ...
    # Generate detailed test results and save them to a file
