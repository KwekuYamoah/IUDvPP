...
def test_model(model, iterator):
    ...
    with open('./outputs/raw_transformer_multiclass_results.txt', 'w') as file:
        file.write("")
    ...
    with torch.no_grad():
        for batch_idx, (words, features, labels, lengths) in enumerate(iterator):
            ...
            for i in range(features.size(0)):
                ...
                with open('./outputs/raw_transformer_multiclass_results.txt', 'a') as file:
...
def plot_metrics(train_losses, val_losses, val_accuracies, val_precisions, val_recalls, val_f1s):
    ...
    plt.tight_layout()
    plt.savefig('./outputs/raw_transformer_multiclass_metrics.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    with open(f"{log_dir}/raw_classwise_metrics.txt", "w") as f:
    ...
    with open(f"{log_dir}/raw_confusion_matrix.txt", "w") as f:
        for i, class_name in enumerate(class_names):
