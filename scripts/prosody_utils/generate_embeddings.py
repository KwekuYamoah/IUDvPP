""" 
    ...
    This module provides utilities for generating and normalizing word embeddings using the OpenAI API.
    It loads a JSON dataset containing word-level features, computes embeddings for each word, normalizes
    them using L2 norm, and saves the results to a new JSON file. The workflow includes reading input data,
    generating and normalizing embeddings, and writing the enriched data back to disk.
    Functions:
        - load_json: Load a JSON file containing word-level data.
        - get_embedding: Generate an embedding for a given text using OpenAI's embedding models.
        - normalize_l2: Normalize vectors or matrices using the L2 norm.
        - generate_embeddings: Generate and normalize embeddings for all words in the dataset.
        - write_json: Write the processed data with embeddings to a JSON file.
