"""
Prosody Multiclass Classification using BiLSTM with Attention and Word Embeddings with Hyperparameter Optimization using Optuna
...
Additionally, it integrates Optuna for hyperparameter optimization to maximize the F1 score,
leading to better class-wise performance. The script incorporates mixed precision training,
efficient data loading, gradient accumulation, and GPU temperature and memory monitoring
...
MAX_MEMORY_USAGE = 14000  # in MB (14 GB)
...
class ProsodyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            tuple: (key, processed_words, combined_features, word_embeddings, labels)
        """
        key, item = self.entries[idx]
        processed_words = preprocess_text(item['words'])
        
        # Load prosody features
        prosody_features = torch.tensor(item['features'], dtype=torch.float32)  # Shape: (seq_len, prosody_dim)
        
        # Load word embeddings
        word_embeddings = torch.tensor(item['word_embeddings'], dtype=torch.float32)  # Shape: (seq_len, 300)
        
        # Concatenate prosody features with word embeddings
        combined_features = torch.cat((prosody_features, word_embeddings), dim=1)  # Shape: (seq_len, prosody_dim + 300)
...
class AttentionBasedFusion(nn.Module):
    ...
    def forward(self, combined_features, mask):
        ...
        attn_weights = torch.softmax(attention.masked_fill(~mask, -1e4), dim=1).unsqueeze(2)  # (batch, seq_len, 1)
...
def initialize_gpu_monitor():
    """
    Initializes the GPU monitoring by starting pynvml.
    ...
    try:
        ...
        raise RuntimeError(f"Failed to initialize NVML: {error}")
...
def get_gpu_stats():
    ...
    Returns:
        tuple: (temperature in Celsius, memory used in MB)
    """
    handle = pynvml.nvmlDeviceGetHandleByIndex(0)  # Assuming single GPU
    temperature = pynvml.nvmlDeviceGetTemperature(handle, pynvml.NVML_TEMPERATURE_GPU)
    mem_info = pynvml.nvmlDeviceGetMemoryInfo(handle)
    memory_used = mem_info.used // 1024 // 1024  # Convert bytes to MB
    return temperature, memory_used

def check_gpu_limits():
    """
    Checks if the GPU temperature and memory usage are within specified limits.

    Returns:
        bool: True if within limits, False otherwise.
    """
    temperature, memory_used = get_gpu_stats()
    if temperature > MAX_TEMPERATURE:
        print(f"GPU temperature {temperature}°C exceeds the limit of {MAX_TEMPERATURE}°C.")
        return False
    if memory_used > MAX_MEMORY_USAGE:
        print(f"GPU memory usage {memory_used}MB exceeds the limit of {MAX_MEMORY_USAGE}MB.")
        return False
    return True
...
def train_model(model, iterator, optimizer, criterion, device, num_classes, scaler, accumulation_steps=1):
    """
    Trains the model for one epoch with mixed precision and gradient accumulation.
    ...
    epoch_loss = 0
    optimizer.zero_grad()
    for step, (keys, words, features, word_embeddings, labels, lengths) in enumerate(iterator):
...
def test_model(model, iterator, device, num_classes):
    ...
    results_json_path = os.path.join('../outputs/prosody_bilstm_features_multiclass_results.json')
...
def plot_metrics(train_losses, val_losses, val_accuracies, val_precisions, val_recalls, val_f1s, study=None):
    """
    ...
    Args:
        ...
        study (optuna.study.Study, optional): The Optuna study object for additional annotations. Defaults to None.
    """
...
def evaluate_new_set(model, new_dataset_path, scaler=None, device='cpu', num_classes=3):
    ...
    new_loader = DataLoader(new_dataset, batch_size=8, shuffle=False, collate_fn=collate_fn, num_workers=7, pin_memory=True)
...
def main():
    ...
    train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True, 
                              collate_fn=collate_fn, num_workers=7, pin_memory=True)
    ...
    sample_keys, sample_words, sample_features, sample_word_embeddings, sample_labels, sample_lengths = next(iter(train_loader))
    feature_dim = sample_features.shape[2]  # Combined feature dim (prosody + word embeddings)
    ...
    def objective(trial: Trial):
        ...
        Returns:
            float: The best F1 score on the validation set.
        """
        # ==============================
        # Hyperparameters to Tune
        # ==============================

        PROJECTED_DIM = trial.suggest_int('projected_dim', 128, 512, step=64)  # Fixed projection dimension
        HIDDEN_DIM = trial.suggest_int('hidden_dim', 128, 512, step=64)
        NUM_LAYERS = trial.suggest_int('num_layers', 2, 6)
        DROPOUT = trial.suggest_float('dropout', 0.1, 0.5, step=0.05)
        NUM_ATTENTION_LAYERS = trial.suggest_int('num_attention_layers', 1, 4)
        LR = trial.suggest_float('lr', 1e-5, 1e-2, log=True)
        WEIGHT_DECAY = trial.suggest_float('weight_decay', 1e-6, 1e-3, log=True)
        PATIENCE = trial.suggest_int('patience', 5, 15)
        MIN_DELTA = trial.suggest_float('min_delta', 0.0001, 0.01, log=True)
        ACCUMULATION_STEPS_TRIAL = trial.suggest_int('accumulation_steps', 1, 4)  # Optional

        # ==============================
        # Check GPU Limits Before Starting Trial
        # ==============================

        if not check_gpu_limits():
            raise optuna.exceptions.TrialPruned("GPU limits exceeded before trial start.")
        ...
        # Initialize Encoder, Fusion, and Decoder with hyperparameters
        ...
        decoder = Decoder(hidden_dim=HIDDEN_DIM, output_dim=num_classes, num_layers=NUM_LAYERS, 
        ...
        # Initialize optimizer with suggested hyperparameters
        ...
        # Initialize early stopping with suggested hyperparameters
        ...
        for epoch in range(N_EPOCHS):
            # Check GPU limits
            if not check_gpu_limits():
                print("GPU limits exceeded during trial. Pruning the trial.")
                raise optuna.exceptions.TrialPruned("GPU limits exceeded during trial.")

            train_loss = train_model(model, train_loader, optimizer, criterion, device, num_classes, scaler, 
                                     accumulation_steps=ACCUMULATION_STEPS_TRIAL)
            ...
            if valid_loss < best_valid_loss:
                ...
                # Optionally, save the model state dict here if needed
        ...
        return max(val_f1s)  # Return the best F1 score achieved
    ...
    # Optimize the study
    study.optimize(objective, n_trials=100, timeout=None, gc_after_trial=True)
    ...
    os.makedirs('../outputs', exist_ok=True)
    study_summary = study.trials_dataframe().to_csv('../outputs/optuna_study_summary_multiclass.csv', index=False)
    print("Optuna study summary saved to '../outputs/optuna_study_summary_multiclass.csv'")
